...
def scene_objects():
	# getting the version of blender
	new_principled = False
	...
	for ob_name in ('eyeL', 'eyeR', 'mouth'):
		...
		subsurface_input = "Subsurface"
		sepecular_input = 'Specular'
		mix_rgb_node = None

		if new_principled:
			subsurface_input = 9
			sepecular_input = 12
			# creating the mix color node for the new principled shader
			mix_rgb_node = blenderMaterial.node_tree.nodes.new("ShaderNodeMix")
			mix_rgb_node.data_type = "RGBA"
			# setting up the inputs
			blenderMaterial.node_tree.links.new(TRM_Subsurface.outputs['Subsurface'],
											   mix_rgb_node.inputs[0])
			blenderMaterial.node_tree.links.new(TRM_Subsurface.outputs['Subsurface Color'],
												mix_rgb_node.inputs[7])




		blenderMaterial.node_tree.links.new(TRM_Subsurface.outputs['Subsurface'], principled.inputs[subsurface_input])
		if not new_principled:
			blenderMaterial.node_tree.links.new(TRM_Subsurface.outputs['Subsurface Color'],
												principled.inputs['Subsurface Color'])
		blenderMaterial.node_tree.links.new(SRM_Seperator.outputs['Specular'], principled.inputs[sepecular_input])
		...
		if ob_name == 'eyeR' or ob_name == 'eyeL':
			for image_name in ('eye_tex.png', 'eye_tex_nrm.png', 'eye_tex_srm.png'):
				...
				# adding variation for new principled
				principled_base_color_input = blenderTexture.outputs['Color']
				if new_principled and image_name == 'eye_tex.png':
					blenderMaterial.node_tree.links.new(blenderTexture.outputs['Color'],
														mix_rgb_node.inputs[6])
					principled_base_color_input = mix_rgb_node.outputs['Result']
				...
				if blenderImage.name == 'eye_tex.png':
					...
					blenderMaterial.node_tree.links.new(principled_base_color_input, principled.inputs['Base Color'])
			...
			for image_name in ('oral_bsm.tga', 'oral_nrm.tga', 'oral_srm.tga', 'oral_trm.tga'):
				...
				# adding variation for new principled
				principled_base_color_input = blenderTexture.outputs['Color']
				if new_principled and image_name == 'oral_bsm.tga':
					blenderMaterial.node_tree.links.new(blenderTexture.outputs['Color'],
														mix_rgb_node.inputs[6])
					principled_base_color_input = mix_rgb_node.outputs['Result']
				blenderImage = bpy.data.images[image_name]
...
class FMDL_Scene_Extract_Fpk(Operator, ImportHelper):
...
class FMDL_Scene_Open_Image(Operator, ImportHelper):
